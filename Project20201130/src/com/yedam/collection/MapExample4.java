package com.yedam.collection;

import java.util.TreeSet;

public class MapExample4 {
	public static void main(String[] args) { // TreeSet은 크다 작다 기준이 있어야 담을 수 있음
		TreeSet<Integer> tset = new TreeSet<>(); // 작은 수- 큰 수 순으로 정리함.
		tset.add(50);
		tset.add(30);
		tset.add(45);
		tset.add(55);
		
		System.out.println(tset.first());
		System.out.println(tset.last());
		
		TreeSet<String> tstr = new TreeSet<>(); // 가나다 순으로 정리함.
     	tstr.add("Hong");
     	tstr.add("Hwang");
     	tstr.add("Park");
     	tstr.add("Choi");
     	
     	System.out.println(tstr.first());
		System.out.println(tstr.last());
		
		TreeSet<Student> tstu = new TreeSet<>(); //컴페어러블한 타입으로 변경이 불가능하다는 오류 뜸. // 메소드 정의해서 수정 완료.
		tstu.add(new Student(100, "Hong", 70));
		tstu.add(new Student(101, "Hwang", 80));
		tstu.add(new Student(102, "Park", 90));

		System.out.println(tstu.first().getStudentNo()+","+tstu.first().getStudentName());
		System.out.println(tstu.last().getStudentNo()+","+tstu.last().getStudentName());
	}

}
